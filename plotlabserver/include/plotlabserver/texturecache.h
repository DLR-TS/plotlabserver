 ...
 *  - start "D:\Dominion\User\Extern\cmake-3.8.2_win32_x86\bin\cmake-gui.exe"
    ...
    - Build directory:
"D:\Dominion\User\Extern\curl-7.60.0/build"
...
*	DLLS:
*   - curl may be built against zlib: copy libcurl(-d).dll and zlib1.dll to
working directory
*
 */
class TextureCache {
 private:
  ...
  GLuint fromHTTP(const std::string &url) {
    ...
    // gis url:
    // https://maps.aim.ts.dlr.de/geoserver/bs.amt/wms?service=WMS&version=1.1.0&request=GetMap&layers=bs.amt:DLK_2017&styles=&bbox=594999.975,5782000.025,612999.9749999995,5802000.025&width=691&height=768&srs=EPSG:25832&format=image%2Fpng
    if (bboxpos == std::string::npos ||
        widthpos == std::string::npos)  // an arbitrary url
    {
 ...
 public:
  ...
  void setLoadFromHTTP(bool value) { m_loadfromHTTP = value; }
  virtual ~TextureCache() {
    // curl_easy_cleanup(m_curl);
  }
  /**
   *	getGLTexture: retrieves an image from a specified file, saves it for
   *future use and returns an index of the image
   */
