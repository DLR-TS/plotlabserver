...
class MStream {
 ...
 public:
  ...
  void generateHeader() {
    ...
    *out << "% set the global activate_plotting to false, if you want to read "
            "the data without plotting."
         << std::endl;
    *out << "%% -------------------------------------------" << std::endl;
    *out << "global activate_plotting;" << std::endl;
    *out << "if isempty(activate_plotting);activate_plotting=1;end"
         << std::endl;
    *out << "%% -------------------------------------------" << std::endl;
    ...
    *out << "\th = plot3(" << xname << "," << yname << "," << zname;
    if (linecolor != 0)
      *out << ",'Color',[" << linecolor[0] << "," << linecolor[1] << ","
           << linecolor[2] << "]";
    if (pointsize != 0.0)
      *out << ",'MarkerSize'," << pointsize << ",'Marker','.'";
  ...
  void generatePatch(const std::string &hashtag, const std::string &xname,
                     const std::string &yname, const std::string &zname,
                     double *fillcolor, double *linecolor = 0,
                     double pointsize = 0.0, bool linestyle_none = false,
                     double linewidth = 1.0) {
    *out << "if activate_plotting" << std::endl;
    *out << "\th = patch(" << xname << "," << yname << "," << zname << ",["
