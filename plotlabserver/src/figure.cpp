...
double Figure::nexttic(double val, double delta, int direction) {
  return direction >= 0 ? (double)ceil(val / delta) * delta
                        : (double)floor(val / delta) * delta;
}

int Figure::getGlutFigureHandle() { return glut_figure_handle; }
void Figure::display(void) {
  ...
  int stepsX = (int)(floor(((double)(w - 2 * axis.borderWidthPx)) /
                           ((double)(axis.gridWidthPx))));
  int stepsY = (int)(floor(((double)(h - 2 * axis.borderWidthPx)) /
                           ((double)(axis.gridWidthPx))));
  ...
  if (applyViewPortOffsets) {
    ...
    glOrtho(-leftRightPlanes, +leftRightPlanes, -topBottomPlanes,
            +topBottomPlanes, r, 3.0 * r);
  } else {
    if (axis._auto) {
      // axis auto
      ...
      glOrtho(-0.5 * (fmaxx - fminx) * (1 - transZ),
              0.5 * (fmaxx - fminx) * (1 - transZ),
              -0.5 * (fmaxy - fminy) * (1 - transZ),
              0.5 * (fmaxy - fminy) * (1 - transZ), r, 3.0 * r);
    } else {
      // axis equal
      double aspect_ratio = (double)this->h / (double)this->w;
      r = mymax(fmaxx - fminx, fmaxy - fminy, dz) / 2.0;
      glFrustum(-0.5 * r * (1 - transZ), +0.5 * r * (1 - transZ),
                -0.5 * r * aspect_ratio * (1 - transZ),
                +0.5 * r * aspect_ratio * (1 - transZ), r, 3 * r);
    }
  }
  ...
  // std::cout << " debug plotobject count " << buffer.plotObjects.size() +
  // buffer.deferredObjects.size() << " - ( " << buffer.deferredObjects.size()
  // << " deferred )" << std::endl;
  for (auto it = buffer.plotObjects.begin(); it != buffer.plotObjects.end();
       it++) {
    //(*it)->display();
    it->second->display(-xmid, -ymid, -zmid);
  }
  for (auto it = buffer.deferredObjects.begin();
       it != buffer.deferredObjects.end(); it++) {
    //(*it)->display();
    it->second->display(-xmid, -ymid, -zmid);
  }
  ...
  if (logPosition()) {
    GLdouble z;
    glReadPixels(mouseOverX, h - mouseOverY, 1, 1, GL_DEPTH_COMPONENT,
                 GL_DOUBLE, &z);
    ...
    gluUnProject(mouseOverX, h - mouseOverY, 0, model_mat, proj_mat, view_mat,
                 &objX, &objY, &objZ);

    GLdouble obj2X;
    GLdouble obj2Y;
    GLdouble obj2Z;
    gluUnProject(mouseOverX, h - mouseOverY, 1, model_mat, proj_mat, view_mat,
                 &obj2X, &obj2Y, &obj2Z);
    ...
    std::stringstream ss;
    ss << std::setprecision(getPrecision(resultant.x_)) << "x: " << resultant.x_
       << " - y: " << std::setprecision(getPrecision(resultant.y_))
       << resultant.y_
       << " - z: " << std::setprecision(getPrecision(resultant.z_))
       << resultant.z_;
  ...
  if (showGrid) {
    ...
    for (int i = 1; i < stepsX_real; i++) {
      glVertex3d(minx_grid + xgridWidth * (double)i - xmid, miny_grid - ymid,
                 0.0);
      glVertex3d(minx_grid + xgridWidth * (double)i - xmid, maxy_grid - ymid,
                 0.0);
    }
    for (int i = 1; i < stepsY_real; i++) {
      glVertex3d(minx_grid - xmid, miny_grid + ygridWidth * (double)i - ymid,
                 0.0);
      glVertex3d(maxx_grid - xmid, miny_grid + ygridWidth * (double)i - ymid,
                 0.0);
    }
    glEnd();
  }
  ...
  if (showAxis) {
    ...
    for (int i = 0; i < stepsX_real + 0.01; i++) {
      ...
      double fy = miny_grid;
      plotLabel(ss.str().c_str(), ss.str().length(), fx - xmid, fy - ymid,
                1.0 / sx2px, 1.0 / sy2px, 1, 0);
    }
    // y-numbers
    ...
    for (int i = 0; i < stepsX_real + 0.01; i++) {
      ...
      glVertex3d(minx_grid + xgridWidth * (double)i - xmid,
                 miny_grid - ly / 2.0 - ymid, 0);
      glVertex3d(minx_grid + xgridWidth * (double)i - xmid,
                 miny_grid + ly / 2.0 - ymid, 0);
      // north
      glVertex3d(minx_grid + xgridWidth * (double)i - xmid,
                 maxy_grid - ly / 2.0 - ymid, 0);
      glVertex3d(minx_grid + xgridWidth * (double)i - xmid,
                 maxy_grid + ly / 2.0 - ymid, 0);
    }
    for (int i = 0; i < stepsY_real + 0.01; i++) {
      // west
      glVertex3d(minx_grid - lx / 2.0 - xmid,
                 miny_grid + ygridWidth * (double)i - ymid, 0);
      glVertex3d(minx_grid + lx / 2.0 - xmid,
                 miny_grid + ygridWidth * (double)i - ymid, 0);
      // east
      glVertex3d(maxx_grid - lx / 2.0 - xmid,
                 miny_grid + ygridWidth * (double)i - ymid, 0);
      glVertex3d(maxx_grid + lx / 2.0 - xmid,
                 miny_grid + ygridWidth * (double)i - ymid, 0);
    }
  ...
  // plot labels
  if (showAxis) {
    plotLabel(xlabel.c_str(), xlabel.length(),
              (maxx_grid + minx_grid) / 2.0 - xmid,
              (fminy + miny_grid) / 2.0 - ymid, 1.0 / sx2px, 1.0 / sy2px, 0, 0);
    plotLabel(ylabel.c_str(), ylabel.length(), (fminx + minx_grid) / 2.0 - xmid,
              (maxy_grid + miny_grid) / 2.0 - ymid, 1.0 / sx2px, 1.0 / sy2px, 0,
              0);
    plotLabel(title.c_str(), title.length(),
              (maxx_grid + minx_grid) / 2.0 - xmid,
              (fmaxy + maxy_grid) / 2.0 - ymid, 1.0 / sx2px, 1.0 / sy2px, 0, 0);
  }
  ...
  // sleep: without sleep, this thread tends to block the data receive thread
  // under high load
  usleep(50);
}
void Figure::plotLabel(const char *s, int length, double x, double y, double sx,
...
void Figure::plotLabelUI(std::string s, double x, double y,
  ...
  glutBitmapString(GLUT_BITMAP_8_BY_13,
                   reinterpret_cast<unsigned const char *>(text));
  glMatrixMode(GL_MODELVIEW);
  glPopMatrix();
  glMatrixMode(GL_PROJECTION);
  glPopMatrix();
...
void Figure::toMatlabFile() {
  ...
  (*mstream->getOStream()) << "set(figure(" << id << "),'Name','" << name
                           << "');" << std::endl;
...
void Figure::setViewPortOffsets(double x, double y, double orientDeg,
                                double zoom, bool disable) {
  if (disable) {
    applyViewPortOffsets = false;
...
void Figure::mouseFunction(int button, int state, int x, int y) {
  ...
  switch (button) {
        ...
        if (((double)(now - t_mouseDown)) / CLOCKS_PER_SEC < 0.2) {
          ...
          if (click_count == 1) {
            if (logPosition()) {
              ...
              logPositionFile << std::fixed << resultant.x_ << ","
                              << resultant.y_ << "," << resultant.z_;
              logPositionFile << std::fixed << "," << h.heading_;
              logPositionFile << std::fixed << "," << q.x_ << "," << q.y_ << ","
                              << q.z_ << "," << q.w_;
    ...
    case 3:
      ...
      transZ = std::min(0.99999997f,
                        static_cast<float>(transZ + (1 - transZ) * 0.1));
      glutPostRedisplay();
      break;
    case 4:
      if (state == GLUT_UP) return;
...
void Figure::setShowAxis(bool value) { showAxis = value; }
void Figure::setShowGrid(bool value) { showGrid = value; }
void Figure::setLogPosition(bool value) {
  if (value && !logPosition()) {
    logPositionFile.open("logged_positions.txt",
                         std::ofstream::out | std::ofstream::app);
    logPositionFile << "begin_record" << std::endl;
    logPositionFile << "point_x, point_y, point_z, heading, quaternion_x, "
                       "quaternion_y, quaternion_z, quaterion_w"
                    << std::endl;
...
bool Figure::logPosition() { return logPositionFile.is_open(); }
void Figure::setXLabel(std::string value) { xlabel = value; }
void Figure::setYLabel(std::string value) { ylabel = value; }
void Figure::setZLabel(std::string value) { zlabel = value; }
void Figure::setTitle(std::string value) { title = value; }
void Figure::setName(std::string value) {
  name = value;
  nameChanged = true;
}
/**
 *	add_nomutex: this method is private and grants access without mutex -
 *use only within mutex block
 */
...
void Figure::add(
    const std::vector<std::pair<std::string, PlotObject *>> &objects) {
  if (objects.size() > 0) {
    pthread_mutex_lock(&(buffer.mut));
...
void Figure::append(
    const std::vector<std::pair<std::string, LinePlot *>> &objects) {
  if (objects.size() > 0) {
    pthread_mutex_lock(&(buffer.mut));
    for (auto it = objects.begin(); it != objects.end(); it++) {
...
void Figure::setFixedLimits(double x0, double y0, double z0, double x1,
                            double y1, double z1) {
  pthread_mutex_lock(&(buffer.mut));
  buffer.fixed_limits = true;
  bound_recomputation_necessary = false;
  buffer.minx = x0;
  buffer.miny = y0;
  buffer.minz = z0;
...
void Figure::setAutoLimits() {
  pthread_mutex_lock(&(buffer.mut));
  buffer.fixed_limits = false;
  bound_recomputation_necessary = true;
  pthread_mutex_unlock(&(buffer.mut));
}
void Figure::recomputeBounds() { bound_recomputation_necessary = true; }
void Figure::recomputeBounds_execute() {
  ...
  if (!freezeFocus && !buffer.fixed_limits) {
    pthread_mutex_lock(&(buffer.mut));
    buffer.minx = 9e99;
    buffer.miny = 9e99;
    buffer.minz = 9e99;
...
void Figure::clear() { buffer.clear(); }

void Figure::drawnow() { needsPostRedisplay = true; }

bool Figure::loadFromHTTP = true;

void *pthread_glutMainLoop_wrapper(void *id) {
  // char* argv[] = {""}; // this gives a warning, leaving it in as a reminder
  // if we ever need argv here
  int argc = 1;
  glutInit(&argc, 0);
  glutInitDisplayMode(GLUT_RGBA | GLUT_ALPHA | GLUT_DOUBLE | GLUT_DEPTH);
  glutInitWindowSize(300, 300);

  /* Create windows before glutMainLoop, then hide them: Seems impossible to
   * create windows after entering glutMainLoop*/
...
void Figure::init() {
  ...
  int threadid;
  int retval =
      pthread_create(&thread, NULL, pthread_glutMainLoop_wrapper, &threadid);
...
TextureCache *Figure::getTextureCache() { return &textureCache; }
...
void Figure::updateFunction(void) {
  ...
  for (std::map<unsigned int, Figure *>::iterator it =
           static_figure_set.begin();
       it != static_figure_set.end(); it++) {
    Figure *f = it->second;
    if (f->needsPostHide || f->needsPostShow || f->needsPostRedisplay ||
        f->nameChanged) {
...
void Figure::glut_reshape0(int w, int h) {
  static_figure_set.at(0)->reshape(w, h);
}
void Figure::glut_reshape1(int w, int h) {
  static_figure_set.at(1)->reshape(w, h);
}
void Figure::glut_reshape2(int w, int h) {
  static_figure_set.at(2)->reshape(w, h);
}
void Figure::glut_reshape3(int w, int h) {
  static_figure_set.at(3)->reshape(w, h);
}
void Figure::glut_reshape4(int w, int h) {
  static_figure_set.at(4)->reshape(w, h);
}
void Figure::glut_reshape5(int w, int h) {
  static_figure_set.at(5)->reshape(w, h);
}
void Figure::glut_reshape6(int w, int h) {
  static_figure_set.at(6)->reshape(w, h);
}
void Figure::glut_reshape7(int w, int h) {
  static_figure_set.at(7)->reshape(w, h);
}
void Figure::glut_reshape8(int w, int h) {
  static_figure_set.at(8)->reshape(w, h);
}
void Figure::glut_reshape9(int w, int h) {
  static_figure_set.at(9)->reshape(w, h);
}
void Figure::glut_motionFunc0(int x, int y) {
  static_figure_set.at(0)->motionFunction(x, y);
}
void Figure::glut_motionFunc1(int x, int y) {
  static_figure_set.at(1)->motionFunction(x, y);
}
void Figure::glut_motionFunc2(int x, int y) {
  static_figure_set.at(2)->motionFunction(x, y);
}
void Figure::glut_motionFunc3(int x, int y) {
  static_figure_set.at(3)->motionFunction(x, y);
}
void Figure::glut_motionFunc4(int x, int y) {
  static_figure_set.at(4)->motionFunction(x, y);
}
void Figure::glut_motionFunc5(int x, int y) {
  static_figure_set.at(5)->motionFunction(x, y);
}
void Figure::glut_motionFunc6(int x, int y) {
  static_figure_set.at(6)->motionFunction(x, y);
}
void Figure::glut_motionFunc7(int x, int y) {
  static_figure_set.at(7)->motionFunction(x, y);
}
void Figure::glut_motionFunc8(int x, int y) {
  static_figure_set.at(8)->motionFunction(x, y);
}
void Figure::glut_motionFunc9(int x, int y) {
  static_figure_set.at(9)->motionFunction(x, y);
}
void Figure::glut_passiveMotionFunc0(int x, int y) {
  static_figure_set.at(0)->passiveMotionFunction(x, y);
}
void Figure::glut_passiveMotionFunc1(int x, int y) {
  static_figure_set.at(1)->passiveMotionFunction(x, y);
}
void Figure::glut_passiveMotionFunc2(int x, int y) {
  static_figure_set.at(2)->passiveMotionFunction(x, y);
}
void Figure::glut_passiveMotionFunc3(int x, int y) {
  static_figure_set.at(3)->passiveMotionFunction(x, y);
}
void Figure::glut_passiveMotionFunc4(int x, int y) {
  static_figure_set.at(4)->passiveMotionFunction(x, y);
}
void Figure::glut_passiveMotionFunc5(int x, int y) {
  static_figure_set.at(5)->passiveMotionFunction(x, y);
}
void Figure::glut_passiveMotionFunc6(int x, int y) {
  static_figure_set.at(6)->passiveMotionFunction(x, y);
}
void Figure::glut_passiveMotionFunc7(int x, int y) {
  static_figure_set.at(7)->passiveMotionFunction(x, y);
}
void Figure::glut_passiveMotionFunc8(int x, int y) {
  static_figure_set.at(8)->passiveMotionFunction(x, y);
}
void Figure::glut_passiveMotionFunc9(int x, int y) {
  static_figure_set.at(9)->passiveMotionFunction(x, y);
}
void Figure::glut_mouseFunc0(int button, int state, int x, int y) {
  static_figure_set.at(0)->mouseFunction(button, state, x, y);
}
void Figure::glut_mouseFunc1(int button, int state, int x, int y) {
  static_figure_set.at(1)->mouseFunction(button, state, x, y);
}
void Figure::glut_mouseFunc2(int button, int state, int x, int y) {
  static_figure_set.at(2)->mouseFunction(button, state, x, y);
}
void Figure::glut_mouseFunc3(int button, int state, int x, int y) {
  static_figure_set.at(3)->mouseFunction(button, state, x, y);
}
void Figure::glut_mouseFunc4(int button, int state, int x, int y) {
  static_figure_set.at(4)->mouseFunction(button, state, x, y);
}
void Figure::glut_mouseFunc5(int button, int state, int x, int y) {
  static_figure_set.at(5)->mouseFunction(button, state, x, y);
}
void Figure::glut_mouseFunc6(int button, int state, int x, int y) {
  static_figure_set.at(6)->mouseFunction(button, state, x, y);
}
void Figure::glut_mouseFunc7(int button, int state, int x, int y) {
  static_figure_set.at(7)->mouseFunction(button, state, x, y);
}
void Figure::glut_mouseFunc8(int button, int state, int x, int y) {
  static_figure_set.at(8)->mouseFunction(button, state, x, y);
}
void Figure::glut_mouseFunc9(int button, int state, int x, int y) {
  static_figure_set.at(9)->mouseFunction(button, state, x, y);
}
void Figure::glut_keyPressed0(unsigned char key, int x, int y) {
  static_figure_set.at(0)->keyFunction(key, x, y);
}
void Figure::glut_keyPressed1(unsigned char key, int x, int y) {
  static_figure_set.at(1)->keyFunction(key, x, y);
}
void Figure::glut_keyPressed2(unsigned char key, int x, int y) {
  static_figure_set.at(2)->keyFunction(key, x, y);
}
void Figure::glut_keyPressed3(unsigned char key, int x, int y) {
  static_figure_set.at(3)->keyFunction(key, x, y);
}
void Figure::glut_keyPressed4(unsigned char key, int x, int y) {
  static_figure_set.at(4)->keyFunction(key, x, y);
}
void Figure::glut_keyPressed5(unsigned char key, int x, int y) {
  static_figure_set.at(5)->keyFunction(key, x, y);
}
void Figure::glut_keyPressed6(unsigned char key, int x, int y) {
  static_figure_set.at(6)->keyFunction(key, x, y);
}
void Figure::glut_keyPressed7(unsigned char key, int x, int y) {
  static_figure_set.at(7)->keyFunction(key, x, y);
}
void Figure::glut_keyPressed8(unsigned char key, int x, int y) {
  static_figure_set.at(8)->keyFunction(key, x, y);
}
void Figure::glut_keyPressed9(unsigned char key, int x, int y) {
  static_figure_set.at(9)->keyFunction(key, x, y);
}
