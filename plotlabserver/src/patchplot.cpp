...
void PatchPlot::generateMCode(MStream *out, std::string hashtag) {
  ...
  out->generatePatch(hashtag, "tmp(1,:)", "tmp(2,:)", "tmp(3,:)", &FillColor[0],
...
void PatchPlot::generatePCode(PStream *out, int d, std::string hashtag) {
  ...
  out->generateMatrix(hashtag, "tmp", &values[0], 3, size);
  if (d == 3) {
    out->generatePatch(hashtag, "tmp", &FillColor[0], &LineColor[0], pointsize,
                       linestyle_none, LineWidth);
  } else {
    out->generatePatch2D(hashtag, "tmp", &FillColor[0], &LineColor[0],
