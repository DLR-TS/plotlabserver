...
// CircleStrip(double* x,double* y, double* r, double cr, double cg, double cb,
// double,unsigned int size);
// // sr,sg,sb is start color rgb, er,eg,eb is end color rgb
// CircleStrip(double* x,double* y, double* r, double sr, double sg, double sb,
// double er, double eg, double eb,unsigned int size); CircleStrip(double*
// x,double* y,double* z,unsigned int size); ~CircleStrip(); virtual void
// display(double dx,double dy,double dz); virtual void generateMCode(MStream*
// out,std::string hashtag=""); virtual void generatePCode(PStream* out,int
// d,std::string hashtag="");
namespace DLR {
namespace PlotLab {
...
CircleStrip::CircleStrip(double *v1, double *v2, double *v3,
                         unsigned int size) {
  // reminder: CircleStrip has a special data layout:
  // given a circle with center x,y,z and radius r and color rgba (1 float per
  // channel) v1[0] = x  v2[0] = y             v3[0] = z v1[1] = r  v2[1] = rg
  // (2 floats) v3[1] = ba (2 floats) then the pattern repeats

  double minx = 9e8, miny = 9e8, minz = 9e8, maxx = -9e8, maxy = -9e8,
         maxz = -9e8, maxr = 0;
  ...
  if (this->size * circleDataSize > pointsBufferSize) {
    std::runtime_error(
        "Got A CircleStrip with too many circles to fit in the buffer");
  }
  ...
  // std::cout << " x " << v1[0] << " \ty " << v2[0] << " z " << v3[0] <<
  // std::endl; std::cout << " r " << v1[1] << " \tr " << drg.fvalue[0] << " \tg
  // " << drg.fvalue[1] << " \tb " << dba.fvalue[0] << " \ta " << dba.fvalue[1]
  // << std::endl; drg.dvalue = v2[size - 1]; dba.dvalue = v3[size - 1];
  // std::cout << " #DEBUG circlestrip.cpp - last circle" << std::endl;
  // std::cout << " x " << v1[size-2] << " \ty " << v2[size-2] << " z " <<
  // v3[size-2] << std::endl; std::cout << " r " << v1[size-1] << " \tr " <<
  // drg.fvalue[0] << " \tg " << drg.fvalue[1] << " \tb " << dba.fvalue[0] << "
  // \ta " << dba.fvalue[1] << std::endl;

  for (unsigned int circle_index = 0; circle_index < this->size;
       circle_index++) {
  ...
  setBoundMax(
      maxx + maxr, maxy + maxr,
      maxz);  // approximative bounds for x and y, but should be reasonable
...
void CircleStrip::display(double dx, double dy, double dz) {
  for (unsigned int circle_index = 0; circle_index < size; circle_index++) {
    ...
    glColor4d(points_buffer[data_index], points_buffer[data_index + 1],
              points_buffer[data_index + 2], 1.0);
    // std::cout << " ~~ debug colors " << points_buffer[data_index] << " " <<
    // points_buffer[data_index + 1] << " " << points_buffer[data_index  + 2] <<
    // std::endl;
    for (unsigned int i = 0; i < CircleStrip::resolution; i++) {
      // std::cout << " ~~ debug coords " << i << " " <<
      // points_buffer[coord_index + i*3] << " " << points_buffer[coord_index +
      // i*3 + 1] << " " << points_buffer[coord_index + i*3 + 2] << std::endl;
