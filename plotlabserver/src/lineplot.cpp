...
void LinePlot::prepend(double *buffer, unsigned int desired_count) {
  unsigned int count =
      std::min(desired_count,
               PlotObject::BUFFER_SIZE / 3 -
                   size);  // limit the shift not to exceed max number of points
  memmove(&values[count * 3], &values[0],
          sizeof(double) * 3 *
              size);  // shift own values to the right - use memmove instead of
                      // memcpy as buffers are overlapping
  memcpy(&values[0], &buffer[(desired_count - count) * 3],
         sizeof(double) * 3 *
             count);  // copy count*3-many values from buffer to front of
                      // values, taking the last count*3 values from buffer
  size = size + count;
  scanBounds();
}
