...
DLR::PlotLab::TexturePlot::TexturePlot(TextureCache *cache,
                                       const std::string &filename, double x,
                                       double y, double z, double psi, double w,
                                       double l) {
  this->_cache = cache;
  ...
  values[11] = z;
  size = 4;
  double minx = 9e9, miny = 9e9, minz = 9e9, maxx = -9e9, maxy = -9e9,
         maxz = -9e9;
...
void DLR::PlotLab::TexturePlot::display(double dx, double dy, double dz) {
  ...
  glVertex3d(values[3] + dx, values[4] + dy, values[5] + dz);
  glTexCoord2f(1.0f, 1.0f);
  glVertex3d(values[6] + dx, values[7] + dy, values[8] + dz);
  glTexCoord2f(1.0f, 0.0f);
  glVertex3d(values[9] + dx, values[10] + dy, values[11] + dz);
  glTexCoord2f(0.0f, 0.0f);
  glEnd();
  glDisable(GL_TEXTURE_2D);
}
void DLR::PlotLab::TexturePlot::generateMCode(MStream *out,
                                              std::string hashtag) {}
void DLR::PlotLab::TexturePlot::generatePCode(PStream *out, int d,
                                              std::string hashtag) {}
